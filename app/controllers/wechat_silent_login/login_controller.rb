...
module WeChatSilentLogin
  class LoginController < ::ApplicationController
    ...
    def redirect_back_with_error(_code)
      origin = session.delete(:wechat_origin) || "/"
      origin = "/" if origin.to_s.start_with?("/wechat/")
      origin = origin.sub(/[?&](code|state)=[^&]+/, "")
      redirect_to origin
  ...
  module Admin
    class LogsController < ::Admin::AdminController
      requires_plugin ::WeChatSilentLogin::PLUGIN_NAME

      def index
        limit = (params[:limit] || 200).to_i
        offset = (params[:offset] || 0).to_i
        @entries = ::WeChatSilentLogin::Logger.list(limit, offset)
        respond_to do |format|
          format.json { render_json_dump(entries: @entries) }
          format.html { render :index }
        end
      end

      def destroy
        ::WeChatSilentLogin::Logger.clear
        respond_to do |format|
          format.json { render_json_dump(ok: true) }
          format.html { redirect_to "/wechat/admin/logs", notice: "cleared" }
        end
      end
    end
  end
  ...
  class LoginHelper
    ...
    def auto_login_with_openid(openid)
      ...
      if provider.method(:log_on_user).arity.abs >= 4 rescue true
    ...
    def log(msg)
      ::WeChatSilentLogin::Logger.push(msg)
